...
## Forecast variance and prediction intervals
...
### Confidence interval for the conditional mean $\mathbb{E}[y \mid x_0]$
...
### Prediction interval for an individual outcome $y_0$
...
## Model selection: $R^2$, adjusted $R^2$, AIC, BIC
...
### $R^2$ and the garbage-variable problem
...
## Lab: Prediction and Model Selection in R

We work with the `wage1` dataset of 526 U.S.\ workers (introduced in Appendix A). The goal is to (i) fit a sequence of nested wage models, (ii) compare them using $\bar R^2$, AIC, BIC, and an $F$-test, (iii) produce point predictions and the two interval estimates for a specific worker, and (iv) visualise the difference between the confidence band for the mean and the prediction band for an individual.
